...
public class EntityOpossum extends ComplexMobTerrestrial implements ISpecies, INewSkins {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        builder.define(PLAYING_DEAD, false);
        builder.define(JOEYS, 0);
    }
        ...
        if (!this.level().isClientSide()) {
            if (this.isInWater()) {
                ...
                if (!this.isNotMoving() && this.random.nextInt(5) == 0 && this.getDeltaMovement().horizontalDistance() > 0.08) {
                    ...
                    if (level().getBlockState(testblockpos.below()).is(BlockTags.MINEABLE_WITH_SHOVEL))
    ...
    public EntityOpossum getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
        return create_offspring(new EntityOpossum(ModEntity.OPOSSUM.get(), serverWorld));
    }
    ...
    public boolean hurtServer(ServerLevel level, DamageSource damageSource, float amount) {
        if (!this.isPlayingDead()) {
            if (damageSource.getEntity() instanceof Mob aggressor) {
                ...
                this.makeAreaOfEffectCloud(level, this.blockPosition());
    ...
    private void makeAreaOfEffectCloud(Level worldIn, BlockPos pos) {
        ...
        linger_cloud.addEffect(new MobEffectInstance(MobEffects.NAUSEA, 80, 0, true, false));
    ...
    public void readAdditionalSaveData(ValueInput compound){
        ...
        this.setPlayingDead(compound.getBooleanOr("playing_dead", false));
        this.setJoeys(compound.getIntOr("joeys", 0));
    }
    ...
    public static class OpossumBackOffGoal<T extends LivingEntity> extends AvoidEntityGoal<T> {
        ...
        public boolean canUse() {
            ...
            List<T> list = this.taskOwner.level().getEntitiesOfClass(avoidClass, this.taskOwner.getBoundingBox().inflate(avoidDistance, 4f, avoidDistance), this.targetSelector::test);
            if (list.isEmpty()) {
                return false;
            } else {
